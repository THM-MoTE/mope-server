...
class SuggestionProvider(
  ...
  val logSuggestions: String => Seq[Suggestion] => Seq[Suggestion] = { word =>
    suggestions =>
  ...
  val logType: String => Option[TypeOf] => Option[TypeOf] = { name =>
    tpe =>
  ...
  private def toStartsWith(word: String): Sink[Suggestion, Future[Seq[Suggestion]]] = {
    ...
    val suggestionFilter = Flow.fromGraph(GraphDSL.create() { implicit builder =>
      ...
      val zip = builder.add(ZipWith[Seq[Suggestion], Seq[Suggestion], Seq[Suggestion]] {
        _ ++ _
      })
